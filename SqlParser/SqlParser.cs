...
namespace Parser
{
    public class SqlParser : ParserBase
    {
        ...
        private XmlNode GetFromTagXmlNode()
        {
            XmlNode myFromNode =
                ParsedDocument.SelectSingleNode(string.Format(@"{0}/{1}[@{2}='{3}']", cRootXmlNodeName, cTagXmlNodeName,
                                                              cTagTypeXmlAttributeName, FromTag.cTagName));
            if (myFromNode == null)
        ...
        {
            XmlNode myForUpdateNode =
                ParsedDocument.SelectSingleNode(string.Format(@"{0}/{1}[@{2}='{3}']", cRootXmlNodeName, cTagXmlNodeName,
                                                              cTagTypeXmlAttributeName, ForUpdateTag.cTagName));
        ...
        private XmlNode GetWhereTagXmlNode(bool createNew)
        {
            XmlNode myWhereNode =
                ParsedDocument.SelectSingleNode(string.Format(@"{0}/{1}[@{2}='{3}']", cRootXmlNodeName, cTagXmlNodeName,
                                                              cTagTypeXmlAttributeName, WhereTag.cTagName));
        ...
        /// <summary>
        /// Gets From clause of the parsed sql.
        /// </summary>
        public string FromClause
        {
            get
            {
                try
                {
                    XmlNode myFromTagXmlNode = GetFromTagXmlNode();
